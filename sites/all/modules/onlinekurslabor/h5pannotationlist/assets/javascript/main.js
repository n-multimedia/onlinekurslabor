...
(function ($) {
    Drupal.behaviors.h5pannotationlist = {
      ...
      // Your Javascript code goes here
        ...
        {
            var video_object = new Drupal.behaviors.h5pannotationlist.video_object(object, container);
            this.video_object_list.push(video_object);
            return 	this.video_object_list.length;
        },
        /*
		...
		*/
        save_video_objects: function ()
        {
            ...
            H5P.InteractiveVideo.prototype.attach= function()
            {
                ...
                var  video_object=  new Drupal.behaviors.h5pannotationlist.video_object(this, arguments[0]);
							
                //speichert videoobjekt in einer liste
                Drupal.behaviors.h5pannotationlist.addVideoContainerObjectToList( video_object);
                //fuegt dem videoobjekt eine sidebar hinzu
                Drupal.behaviors.h5pannotationlist.addSideBarToVideoObject(video_object)
        ...
        addSideBarToVideoObject: function(video_object)
        {
            ...
            jQuery.each( Drupal.settings.h5p.content, function(json_video_id,some_video_object)
            {
                ...
                the_great_list =
                '<div id="h5pannotationlist_'+json_video_id+'" class="h5pannotationlist">'+
                ...
                jQuery.each( Drupal.behaviors.h5pannotationlist.annotations_sortby, function(index,item)
                {
                    the_great_list+='<input type=radio name=annotations_sortby value="'+item+'">&nbsp;'+
                    Drupal.behaviors.h5pannotationlist.annotations_sortby_label[index]+'</input><br>';
                //'<label for="'+item+'">'+ Drupal.behaviors.h5pannotationlist.annotations_sortby_label[index]+'</label>';
                });
                the_great_list+='</div>'+
                '</div>'; //sortregion

                the_great_list+='<div class="h5pannotationlist_annotation_container"></div> </div><div style="clear:both;"></div>';
                ...
                Drupal.behaviors.h5pannotationlist.addAnnotationsToSidebar(json_video_id,null,video_object);
        ...
        /*fuegt dem container '#h5pannotationlist_'+json_video_id  eine liste der annotationen hinzu*/
        addAnnotationsToSidebar: function(json_video_id, sortby, video_object)
        {
            if(!sortby || !this.annotations_sortby.indexOf(sortby))
                sortby="start";
	
            var sidebar_div = jQuery('#h5pannotationlist_'+json_video_id);
            ...
            sidebar_div.find('.sort_container').find('input[name="annotations_sortby"]').change(function(){
                Drupal.behaviors.h5pannotationlist.addAnnotationsToSidebar(json_video_id, jQuery(this).attr("value"), video_object);
            });
            ...
            '<div class="h5pannotationlist_annotation_item annotation_%s">'+
            ...
            new_annotation_html = '<div class="h5pannotationlist_annotation_container_nowrap">'+new_annotation_html+'</div>'
            //haenge html an
            sidebar_div.find("div.h5pannotationlist_annotation_container").html(new_annotation_html);
            var total_width =   sidebar_div.find("div.h5pannotationlist_annotation_item").width()*all_interactions.length;
            
            sidebar_div.find("div.h5pannotationlist_annotation_container_nowrap").css('width', total_width*1.2);
            //mache nun die zeitmarken in der annotationsliste anspringbar
            Drupal.behaviors.h5pannotationlist.convertElementToVideoSeekable('#h5pannotationlist_'+json_video_id, video_object);
            if(all_interactions.length===0)
                {
                      sidebar_div.find("div.h5pannotationlist_annotation_container").html('<i>Keine Annotationen vorhanden</i>');
                }
...
jQuery( document ).ready(function() {
    Drupal.behaviors.h5pannotationlist.save_video_objects();
//Drupal.behaviors.h5pannotationlist.addSideBar( Drupal.behaviors.h5pannotationlist.getAllVideoObjects());
});
